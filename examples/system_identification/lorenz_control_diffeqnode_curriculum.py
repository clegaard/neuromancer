...
def get_data(nsteps, box, sys):
    ...
    sim['Y'] = sim['Y'][:-1, :]
    sim['X'] = sim['X'][:-1, :]
    sim['Time'] = (np.arange(sim['Y'].shape[0]) * args.ts).reshape(-1, 1)

    nx, nu = X.shape[-1], U.shape[-1]
    x, u, t = sim['X'].reshape(args.nsim, nsteps, nx), sim['U'].reshape(args.nsim, nsteps, nu), sim['Time'].reshape(args.nsim, nsteps, 1)
...
if __name__ == "__main__":
    ...
    fx = MLP(nx + nu, nx, bias=False, linear_map=nn.Linear, nonlin=activations['elu'],
             hsizes=[args.hsize for h in range(args.nlayers)])
    interp_u = lambda tq, t, u: u
    integrator = integrators.DiffEqIntegrator(fx, h=args.ts, interp_u=interp_u)
    ...
    callback = TSCallback(validator, args.logdir, figname='test/lorenz_control_node_diffeq_curriculum.png')
